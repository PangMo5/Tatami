...
|`configure`|`key`, `value`|실험용 TOML의 허용한 설정을 원자적으로 바꿔요.|
...
`take`의 기본값은 `--overlay off`이에요. 수정 가능한 ASS·JSON에 문구를 남기고, 출력은 실제 화면 위아래에 공간을 마련해 내용을 가리지 않아요. `scene`의 실시간 연습 패널은 최종 디자인과 달라요.
