      ...
      ScrollViewReader { proxy in
        ScrollView {
          VStack(alignment: .leading, spacing: 20) {
            ...
            }.font(.system(size: 23, design: .monospaced)).id("prompt")
    ...
    Task {
      ...
      do {
        ...
        if entered == "check copy" {
          ...
          let text=String(localized: "Reading saved revision \(session.story.revision)") + "\n\n" + checks.map {"\($0.passed ? "✓" : "✕") \($0.title)"}.joined(separator:"\n") + "\n\n" + String(localized: "\(checks.filter(\.passed).count) of 3 checks passed.") + "\n"
