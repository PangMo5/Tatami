...
<a id="workspaces"></a>
### ワークスペース
[![作業を切り替える](web/media/ja/workspaces.jpg)](https://pangmo5.dev/Tatami/ja/#demo-workspaces)
...
- **[自動で開くデモ](https://pangmo5.dev/Tatami/ja/#demo-autoopen)：** 作業を開くと割り当てたアプリが起動し、閉じたウインドウも入り直すと開きます。
...
### ウインドウのタイル表示（BSP）
[![新しいウインドウが自然に収まる](web/media/ja/tiling.jpg)](https://pangmo5.dev/Tatami/ja/#demo-tiling)


- **BSP 自動配置：** 最も浅いタイルに新しいウインドウを入れます。
- **キーボード操作：** vim 風の `h`、`j`、`k`、`l` で、方向に合わせてフォーカス・入れ替え・サイズを操作できます。
...
[プロファイルと 2 画面の動画を見る](https://pangmo5.dev/Tatami/ja/#profiles)。
...
### 借りる：二つの作業を組み合わせる
[![会話を横に呼び出す](web/media/ja/borrow.jpg)](https://pangmo5.dev/Tatami/ja/#demo-borrow)
...
[作業が変わっても同じ状態ウインドウを見る](https://pangmo5.dev/Tatami/ja/#demo-shared)。
...
[キーとポインタに合わせたフォーカス移動を見る](https://pangmo5.dev/Tatami/ja/#demo-focus)。
...
[実際の CLI、フック、設定の即時反映を見る](https://pangmo5.dev/Tatami/ja/#automation)。
...
[ショートカットの練習と提案の確認を見る](https://pangmo5.dev/Tatami/ja/#guided-setup)。
...
全項目、既定値、ショートカットの書き方は [docs/CONFIGURATION.md](docs/ja/CONFIGURATION.md) を参照してください。

会議の操作パネルや作業を切り替えたときのピクチャインピクチャなど、アプリ別の動作は[トラブルシューティング](docs/ja/TROUBLESHOOTING.md)を参照してください。
...
[CLI の完全なガイド](docs/ja/CLI.md)、または [pangmo5.dev/Tatami](https://pangmo5.dev/Tatami/ja/cli.html) の Web 版を参照してください。
...
Tatami は Wojciech Kulik の [FlashSpace]（仮想作業の切り替え）と koekeishiya の [yabai]（タイル表示のモデル）を参考にしています。帰属は [NOTICE.md](NOTICE.ja.md)、依存ライセンスは [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) を参照してください。
