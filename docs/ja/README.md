...
Tatami の名前は、和室の床に敷き詰める「畳」に由来します。畳を組み合わせて部屋を整えるように、Mac のウインドウを並べてワークスペースを整えます。
...
複数のアプリを使う作業では、必要なウインドウを探して並べ直す手間がかかります。Tatami はアプリと配置をワークスペースにまとめ、戻ったときに作業を続けられるようにします。

- 執筆、開発、レビューなど、作業ごとにワークスペースを作れます。BSP タイル表示が新しいウインドウの場所を自動で整えます。戻ったときは、使っていたアプリと配置で作業を再開できます。
- プロジェクトのビルドなど CPU に負荷がかかる作業中も、ワークスペースの切り替えやウインドウのフォーカスがすばやく応答します。
- 資料や会話のワークスペースを隣に借りたり、確認し続けたいウインドウを「常に手前」に置いたりできます。借りた配置を変えても、戻した後に変更が残ります。共有アプリに登録すれば、ワークスペースを切り替えてもそのウインドウを常に手前に表示できます。
- macOS ネイティブの設定画面で、ワークスペースとショートカットを設定できます。ディスプレイに応じたプロファイル、TOML、CLI、フックを使えば、同じワークスペースをさらに細かく制御できます。

<a id="why-i-built-tatami"></a>
## Tatami を作った理由

Tatami を作る前は、yabai、AeroSpace、FlashSpace + Rectangle を行き来しながら、自分に合う作業環境を探していました。yabai の速さと機能は気に入っていましたが、使いたい機能の一部には SIP の部分的な無効化が必要でした。当時の AeroSpace のワークスペースの動作は、Mission Control をよく使う私の作業には合いませんでした。FlashSpace では、アプリを隠す方式と、あらかじめアプリを登録した仮想ワークスペースを使う方法が気に入りました。ウインドウの配置には Rectangle を組み合わせていました。

[FlashSpace に貢献する中で](https://github.com/wojciech-kulik/FlashSpace/pulls?q=is%3Apr+author%3APangMo5)、ウインドウの配置とワークスペースを結び付ける[提案](https://github.com/wojciech-kulik/FlashSpace/discussions/9)もしました。メンテナーはワークスペース管理に集中し、タイル表示はほかのツールに任せる方針を選びました。その方針を尊重しつつ、ワークスペースとウインドウ管理を一つのアプリで連携させる方法を探りたいと思い続けていました。

Mission Control を日々使いながら、ワークスペースやウインドウをもっと自在に扱いたいと思っていました。機能だけでなく、操作にすばやく応え、快適に使えることも大切でした。そこから生まれたのが Tatami です。アプリ、配置、ディスプレイ、自動化を連携させ、macOS ネイティブの設定画面から使えます。SIP の変更も必要ありません。
