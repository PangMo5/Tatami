...
Tatami はワークスペースごとに分割方向、比率、全画面表示の状態を記憶します。ウインドウが閉じられたり、順番に開いたりするときは、現在のウインドウで画面を使いながら、まだないウインドウの位置を保持します。再び開いたウインドウはその位置に戻れます。サイズ変更、配置変更、全画面表示の切り替えなどを行うと、現在のウインドウをもとに配置を記憶し直します。

どのウインドウを開き直すかは、macOS と各アプリが決めます。Tatami は開いたウインドウに記憶した配置を適用し、書類そのものは開き直しません。次の macOS 設定は、それぞれ異なる場面で使われます。

- **システム設定 → デスクトップと Dock**の**アプリケーションを終了するときにウインドウを閉じる**：オフにすると、対応するアプリは次の起動時にウインドウを開き直せます。オンの場合は、新しいウインドウが開くことがあります。
- ログアウト、再起動、システム終了のダイアログにある**再ログイン時にウインドウを再度開く**：選択すると、次のログイン時に macOS がアプリとウインドウを開き直します。Tatami の**ログイン時に起動**を有効にするか、ログイン後に Tatami を開くと、配置を適用できます。

Tatami は書類名ではなく、アプリとウインドウの順序で配置を対応付けます。アプリがウインドウを作り直す順序を変えると、別の書類が保存された位置に入ることがあります。新しいウインドウも以前の位置を使えます。ワークスペースの**自動で開く**は、macOS の書類復元とは別にアプリを起動する設定です。
...
キーボードでのウインドウ切り替えは、Tatami の実行中、ワークスペースごとに最近フォーカスした順序を記憶します。「次のウインドウ」のショートカットを押してすぐ離すと、直前に使ったアプリやウインドウに戻ります。修飾キーを押したまま一覧を選べます。キーを離すかキャンセルするまで一覧の順序は変わりません。ワークスペースの切り替えやタイルの並べ替えでも履歴は維持されます。ウインドウ切り替えのジェスチャは配置順に移動します。
