...
여러 앱으로 작업하다 보면 필요한 창을 찾고 다시 배치하는 데도 시간이 들어요. Tatami는 앱과 배치를 작업 공간에 함께 담아, 다시 돌아왔을 때 바로 이어서 작업할 수 있게 해요.

- 글쓰기, 개발, 검토처럼 작업마다 공간을 만들어 보세요. BSP 타일링이 새 창을 열 때마다 자리를 잡아주고, 작업 공간에 돌아오면 사용하던 앱과 배치로 바로 이어서 작업할 수 있어요.
- 프로젝트 빌드처럼 CPU를 많이 쓰는 작업 중에도 작업 공간 전환과 창 포커스가 빠르게 반응해요.
- 참고 자료나 대화가 있는 작업 공간을 옆으로 빌려오거나, 계속 확인할 창을 항상 위에 두세요. 빌려온 공간의 배치를 바꾸면 돌려보내도 변경 내용이 남아요. 공용 앱으로 등록하면 작업 공간을 옮겨도 그 창을 항상 위에 둘 수 있어요.
- 앱의 설정 화면에서 작업 공간과 단축키를 구성하세요. 디스플레이에 맞춘 프로필, TOML, 명령줄 도구, 훅으로 더 세밀하게 다룰 수도 있어요. 모두 같은 작업 공간을 제어하는 방법이에요.

<a id="why-i-built-tatami"></a>
## Tatami를 만든 이유

Tatami를 만들기 전에는 yabai, AeroSpace, FlashSpace + Rectangle 조합을 오가며 제게 맞는 작업 환경을 찾았어요. yabai의 속도와 기능이 마음에 들었지만, 원하는 기능 중 일부는 SIP를 부분적으로 꺼야 했어요. 당시 AeroSpace의 작업 공간 동작은 Mission Control을 자주 쓰는 제 방식과 맞지 않았어요. FlashSpace에서는 앱을 숨기는 방식과, 앱을 미리 등록해 둔 가상 작업 공간을 활용하는 방식이 마음에 들었어요. 창 배치는 Rectangle로 보완하며 사용했어요.

[FlashSpace에 기여하면서](https://github.com/wojciech-kulik/FlashSpace/pulls?q=is%3Apr+author%3APangMo5) 작업 공간과 창 배치를 연결하는 아이디어도 [제안했어요](https://github.com/wojciech-kulik/FlashSpace/discussions/9). 메인테이너는 작업 공간 관리에 집중하고 타일링은 다른 도구에 맡기는 방향을 선택했어요. 그 방향을 존중하면서도, 작업 공간과 창 관리를 하나의 앱에서 함께 다루는 방법을 계속 탐구하고 싶었어요.

Mission Control을 계속 쓰면서 작업 공간과 창을 더 자유롭게 다루고 싶었어요. 기능뿐 아니라, 조작할 때 빠르고 쾌적하게 반응하는 경험도 중요했어요. 그래서 앱, 배치, 디스플레이, 자동화가 함께 동작하는 Tatami를 만들었어요. macOS에 익숙한 설정 화면에서 구성할 수 있고, SIP를 변경할 필요도 없어요.
