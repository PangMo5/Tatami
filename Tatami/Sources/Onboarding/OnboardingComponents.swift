...
// MARK: - OnboardingFocusPointerPractice

struct OnboardingFocusPointerPractice: View {
  ...
  var body: some View {
    VStack(spacing: 14) {
      OnboardingDemoMonitor(
        ...
        status: "MFF \(mouseFollowsFocus ? "On" : "Off") · FFM \(focusFollowsMouse ? "On" : "Off")",
        hud: result,
      ) {
        HStack(spacing: 9) {
          ForEach(Array(windowOrder.enumerated()), id: \.element) { slotIndex, windowID in
            demoWindow(slotIndex: slotIndex, windowID: windowID)
          }
        }
        .frame(height: 220)
      }
      ...
      OnboardingLessonPrompt(
        ...
        title: lessonTitle,
        detail: lessonDetail,
        completed: testedMouseFollowsFocus && testedFocusFollowsMouse,
      )

      OnboardingDemoControlPanel(
        title: "Focus controls",
        detail: "Previous and Next simulate Tatami focus or window-cycle commands. Swap moves the focused tile without changing which window owns focus.",
      ) {
        HStack(spacing: 8) {
          Button("Previous focus") { cycleFocus(by: -1) }
          Button("Next focus") { cycleFocus(by: 1) }
          Button("Swap focused tile") { swapFocusedWindow() }
          Divider().frame(height: 20)
          Button("Reset") { reset() }
          Spacer()
          Label(
            mouseFollowsFocus
              ? "Virtual pointer stays with the focused window"
              : "Virtual pointer keeps its previous position",
            systemImage: mouseFollowsFocus ? "cursorarrow.motionlines" : "cursorarrow",
          )
    ...
    .animation(.spring(response: 0.28, dampingFraction: 0.84), value: focusedWindowID)
    .animation(.spring(response: 0.28, dampingFraction: 0.84), value: pointerSlotIndex)
    .animation(.spring(response: 0.28, dampingFraction: 0.84), value: windowOrder)
  }

  // MARK: Private

  @State private var focusedWindowID = 0
  @State private var pointerSlotIndex = 0
  @State private var result: String?
  @State private var testedMouseFollowsFocus = false
  @State private var testedFocusFollowsMouse = false
  @State private var windowOrder = [0, 1, 2]

  private let windows = [
    (title: "Editor", symbol: "text.document"),
    (title: "Browser", symbol: "safari"),
    (title: "Messages", symbol: "bubble.left.and.bubble.right"),
  ]

  private var lessonStep: String {
    if !testedMouseFollowsFocus { return "Step 1 of 2" }
    if !testedFocusFollowsMouse { return "Step 2 of 2" }
    return "Complete"
  }

  private var lessonTitle: String {
    if !testedMouseFollowsFocus { return "Keep the pointer with the focused window" }
    if !testedFocusFollowsMouse { return "Let focus follow the pointer" }
    return "You separated MFF from FFM"
  }

  private var lessonDetail: String {
    if !testedMouseFollowsFocus {
      return mouseFollowsFocus
        ? "Press Previous or Next to change focus, then Swap to move that same focused tile. The virtual pointer should land at the focused window's new center in both cases."
        : "Turn MFF On, then use Previous, Next, or Swap. With MFF Off, the focused window can change or move while the virtual pointer deliberately stays behind."
    }
    if !testedFocusFollowsMouse {
      return focusFollowsMouse
        ? "Now move your real pointer over another preview window. Its focus ring should follow without a click."
        : "Turn FFM On, then move your real pointer over another preview window. With FFM Off, hovering does not change keyboard focus."
    }
    return "MFF keeps the pointer attached after focus or tile movement. FFM starts from pointer movement and changes keyboard focus afterward."
  }
  ...
  private func demoWindow(
    ...
    let focused = focusedWindowID == windowID
    return Button {
      focusedWindowID = windowID
      pointerSlotIndex = slotIndex
      result = "Clicked \(info.title) · pointer position preserved"
    } label: {
      VStack(spacing: 0) {
        HStack(spacing: 5) {
        ...
        ZStack {
          VStack(spacing: 8) {
            ...
            Text(focused ? "Keyboard focus" : "Not focused")
              .font(.caption.weight(.medium))
              .foregroundStyle(focused ? Color.primary : Color.secondary)
          }
          if pointerSlotIndex == slotIndex {
        ...
        .background(focused ? Color.accentColor.opacity(0.12) : Color.primary.opacity(0.025))
      }
    }
    .buttonStyle(.plain)
    ...
    .overlay {
      RoundedRectangle(cornerRadius: 10)
        .strokeBorder(
          focused ? Color.accentColor.opacity(0.85) : Color.primary.opacity(0.12),
          lineWidth: focused ? 2 : 1,
        )
    }
    .shadow(color: .black.opacity(focused ? 0.2 : 0.1), radius: focused ? 8 : 4, y: 3)
    .onHover { hovering in
      guard hovering else { return }
      pointerSlotIndex = slotIndex
      guard focusFollowsMouse, focusedWindowID != windowID else { return }
      focusedWindowID = windowID
      testedFocusFollowsMouse = true
      result = "FFM focused \(info.title) from pointer movement"
    }
    .accessibilityElement(children: .combine)
    .accessibilityLabel("\(info.title)\(focused ? ", focused" : "")")
    .accessibilityHint("Click to focus normally, or hover while Focus follows mouse is on")
  }

  private func cycleFocus(by offset: Int) {
    guard let focusedSlot = windowOrder.firstIndex(of: focusedWindowID) else { return }
    let nextSlot = (focusedSlot + offset + windowOrder.count) % windowOrder.count
    focusedWindowID = windowOrder[nextSlot]
    let title = windows[focusedWindowID].title
    if mouseFollowsFocus {
      pointerSlotIndex = nextSlot
      testedMouseFollowsFocus = true
      result = "MFF moved the pointer to \(title)"
    } else {
      result = "Focused \(title) · pointer stayed behind"
    }
  }

  private func swapFocusedWindow() {
    guard let focusedSlot = windowOrder.firstIndex(of: focusedWindowID) else { return }
    let targetSlot = focusedSlot == windowOrder.count - 1 ? focusedSlot - 1 : focusedSlot + 1
    windowOrder.swapAt(focusedSlot, targetSlot)
    let title = windows[focusedWindowID].title
    if mouseFollowsFocus {
      pointerSlotIndex = targetSlot
      testedMouseFollowsFocus = true
      result = "MFF stayed with \(title) after Swap"
    } else {
      result = "\(title) moved · pointer stayed in its old tile"
    }
  }
  ...
  private func reset() {
    ...
    pointerSlotIndex = 0
    result = nil
    testedMouseFollowsFocus = false
    testedFocusFollowsMouse = false
    windowOrder = [0, 1, 2]
  }
