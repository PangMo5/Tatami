...
- Keep focus, clicks, and scrolling connected to the real window. Mirrors, animations, and loading states must not trap input or expose stale content. With FFM disabled, mirrors may remain on hover, but the first click or drag must reach the real window without an extra activation click. Retain the initial input until native focus and presentation are verified, then preserve OS gesture ownership through mouse-up. Background scrolling must not require keyboard focus.
