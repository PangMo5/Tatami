...
enum DemoApp: String, CaseIterable {
  ...
  var size: CGSize {
    ...
    case .assistant: CGSize(width: 700, height: 520)
...
private struct TerminalPanel: View {
  ...
  private func line(_ path: String, _ cmd: String) -> some View {
    HStack(spacing: 0) { prompt(path)
      Text(cmd).foregroundStyle(.white)
    }
  }
...
private struct NotesPanel: View {
  var body: some View {
    VStack(alignment: .leading, spacing: 14) {
      Text("Launch notes").font(.title2.weight(.semibold))
      Text("Today").foregroundStyle(.secondary).font(.caption)
      Divider().padding(.vertical, 2)
      NoteRow(text: "Finish the code review", isDone: true)
      NoteRow(text: "Record the Tatami demo", isDone: false)
      NoteRow(text: "Capture Borrow and Shared Apps", isDone: false)
      NoteRow(text: "Publish the release", isDone: false)
      Spacer()
    }
    .padding(26)
    .frame(maxWidth: .infinity, maxHeight: .infinity, alignment: .topLeading)
  ...
  var body: some View {
    HStack(spacing: 10) {
      Image(systemName: isDone ? "checkmark.circle.fill" : "circle")
        .foregroundStyle(isDone ? Color(red: 0.9, green: 0.7, blue: 0.2) : .secondary)
  ...
  var body: some View {
    HStack(spacing: 10) {
      ...
      VStack(alignment: .leading, spacing: 2) {
        ...
        Text("Ready when you need it").font(.caption).foregroundStyle(.secondary)
      }
      Spacer()
      Circle()
        .fill(.green)
...
private struct AssistantConversation: View {
  var body: some View {
    VStack(spacing: 14) {
      AssistantMessage(
        text: "Help me plan a focused coding session.",
        isUser: true,
      )
      AssistantMessage(
        text: "Stay in Code, keep Chat nearby, and borrow it only when you need feedback.",
        isUser: false,
      )
      AssistantMessage(
        text: "I can remain in this scratchpad while your main workspace stays intact.",
        isUser: false,
      )
      Spacer()
    }
  ...
  var body: some View {
    HStack(alignment: .bottom, spacing: 8) {
      ...
      Text(text)
        ...
        .background(
          isUser ? Color.accentColor : Color.accentColor.opacity(0.12),
    ...
    .accessibilityElement(children: .combine)
    .accessibilityLabel(isUser ? "You" : "AI Assistant")
    .accessibilityValue(text)
  }
}

// MARK: - AssistantComposer
...
private struct AssistantComposer: View {
  var body: some View {
    ...
    .accessibilityHint("Ask the AI Assistant anything")
  }
}
      ...
      LazyVGrid(columns: [GridItem(.adaptive(minimum: 150), spacing: 12)], spacing: 12) {
        ForEach(0..<12, id: \.self) { i in
          RoundedRectangle(cornerRadius: 10)
            .fill(LinearGradient(
              colors: swatches[i % swatches.count],
              startPoint: .topLeading,
              endPoint: .bottomTrailing,
            ))
  ...
  private func bubble(_ text: String, incoming: Bool) -> some View {
    HStack {
      ...
      Text(text)
        ...
        .background(
          incoming ? Color(white: 0.5, opacity: 0.22) : Color(red: 0.16, green: 0.59, blue: 1),
