...
<a id="workspaces"></a>
### 工作空間
[![在工作之間切換](web/media/zh-Hant/workspaces.jpg)](https://pangmo5.dev/Tatami/zh-Hant/#demo-workspaces)
...
- **[自動開啟示範](https://pangmo5.dev/Tatami/zh-Hant/#demo-autoopen)：** 啟用工作空間時啟動指派的 App，重新進入時開啟先前關閉的視窗。
...
### 視窗並排（BSP）
[![讓新視窗找到位置](web/media/zh-Hant/tiling.jpg)](https://pangmo5.dev/Tatami/zh-Hant/#demo-tiling)


- **自動 BSP 排列：** 把新視窗插入層級最淺的區域。
- **鍵盤操作：** 用類似 vim 的 `h`、`j`、`k`、`l` 依方向移動焦點、交換與調整大小。
...
[觀看設定組合與雙螢幕示範](https://pangmo5.dev/Tatami/zh-Hant/#profiles)。
...
### 借用：組合兩個工作空間
[![把對話放到旁邊](web/media/zh-Hant/borrow.jpg)](https://pangmo5.dev/Tatami/zh-Hant/#demo-borrow)
...
[觀看跨工作空間的共用狀態視窗](https://pangmo5.dev/Tatami/zh-Hant/#demo-shared)。
...
[觀看焦點跟隨鍵盤和游標](https://pangmo5.dev/Tatami/zh-Hant/#demo-focus)。
...
[觀看實際 CLI、掛鉤與即時設定](https://pangmo5.dev/Tatami/zh-Hant/#automation)。
...
[觀看快速鍵練習與建議檢查](https://pangmo5.dev/Tatami/zh-Hant/#guided-setup)。
...
完整鍵、預設值與快速鍵語法請見 [docs/CONFIGURATION.md](docs/zh-Hant/CONFIGURATION.md)。

關於各 App 的視窗行為，包含浮動會議控制項與跨工作空間的子母畫面，請參閱[疑難排解](docs/zh-Hant/TROUBLESHOOTING.md)。
...
閱讀[完整 CLI 參考](docs/zh-Hant/CLI.md)，或在 [pangmo5.dev/Tatami](https://pangmo5.dev/Tatami/zh-Hant/cli.html) 查看網頁版本。
...
Tatami 受到 Wojciech Kulik 的 [FlashSpace]（虛擬工作空間切換概念）與 koekeishiya 的 [yabai]（視窗並排模型）啟發。歸屬說明見 [NOTICE.md](NOTICE.zh-Hant.md)，相依套件授權見 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。
